const menu = [
  ...
  //   let allButtons="";
  //    allButtons += menuTypeName.map((item)=> {
      
  //   return `
  //     <button id="${item.name}" class="btn btn-outline-dark btn-item" type="button">
  //    ${item.name}
  //     </button>
  //   `
  // });
  ...
  //   buttonSection.insertAdjacentHTML("afterbegin", allButtons);  
  // }
  // createButtons();
  ...
  const listAllMenu=()=>{
    ...
    let allMenu="";
    menuTypeName.map(item=>{
      console.log(item.name)
      allMenu+=createMenu(item)
      
    })

    buttonSection.innerHTML=allMenu;
  }
  listAllMenu();
